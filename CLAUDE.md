...
A cross-browser (Chrome + Firefox) browser extension that operates only on `https://go.xero.com` (the main Xero accounting app; other Xero subdomains are intentionally out of scope). It provides bookkeeping tools and shortcuts for Xero users. The current feature is a popup with checkboxes that hide/unhide predefined regions of the Xero UI via injected CSS.
...
Run a single test file: `pnpm exec vitest run lib/hide-style.test.ts`.
...
- `lib/toggles.ts` — the single source of truth for which page regions can be hidden: `TOGGLES: ElementToggle[]`, each with a stable `id`, display `label`, and CSS `selectors`. **The current selectors are placeholders** — Xero's actual DOM/class names need to be confirmed by inspecting the live app before this is functionally complete. Add new hideable regions here.
- `lib/hide-style.ts` — pure function `buildHideStyle(toggles, state) -> string` that turns toggle state into CSS text (`display: none !important`). Kept free of browser APIs so it's unit-testable without mocking `chrome`/`browser`.
...
**Data flow:** popup checkbox change → `toggleStateItem.setValue()` → `browser.storage.local` → content script's `watch()` callback fires → `buildHideStyle` recomputes → `<style>` tag content replaced. There is no message-passing between popup and content script; storage is the sole channel, so both sides can be developed/tested independently.

**Testing:** `vitest.config.ts` uses `WxtVitest()` from `wxt/testing/vitest-plugin`, which provides WXT's module resolution (`#imports`, auto-imports) and a fake `browser` global inside tests. Only `lib/hide-style.ts` has tests today since it's the only pure logic; UI code in `entrypoints/` is intentionally left thin so most logic can live in and be tested from `lib/`.

## Adding a new hideable element
...
2. Add an entry to `TOGGLES` in `lib/toggles.ts`.
