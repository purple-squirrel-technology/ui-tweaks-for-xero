...
- `lib/menu-items.ts` + `lib/apply-menu-items.ts` — declarative insertion of new menu links into existing Xero menus. Each entry in `MENU_ITEMS` defines target list selector, link text/href, and position (`start`, `end`, `before`, `after` with selector/text references). Toggle state controls insertion/removal.
...
2. Add an entry to `MENU_ITEMS` in `lib/menu-items.ts` with `text`, `href`, and `position`.
